...
include 'header.php';
	...
	if(isset($_GET['id'])){
		$page = $_GET['id'];
		//$query = mysql_query("SELECT nama FROM accounts WHERE username='%$page%'") or die(mysql_error());
		//$row = mysql_fetch_array($query)
		echo "q";
		//$prof = resumeProfile($page);
		//$namaprofile = 
	}else{
    $prof = resumeProfile($usernik);
}
...
<h5 class="mb-1">'.$namaprofile.'</h5>
<p>'.$mms.'</p><p class="mb-1">'.$area.'</p>
</div>
